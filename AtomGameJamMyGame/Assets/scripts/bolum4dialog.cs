...
public class DialogTrigger4 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        dialogCompleted = PlayerPrefs.GetInt("dialogCompleted4", 0) == 1;

        // Kapýnýn durumu
        doorOpened = PlayerPrefs.GetInt("doorOpened4", 1) == 1;
        kapý.SetActive(doorOpened);

        // Eðer s2Unlocked açýldýysa tus ve resim hep aktif kalsýn
        if (PlayerPrefs.GetInt("s3Unlocked", 0) == 1)
        {
            tus.SetActive(true);
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        PlayerPrefs.SetInt("dialogCompleted4", 1);
                        PlayerPrefs.Save();

                        // Silah 1 açýlýyor
                        PlayerPrefs.SetInt("s3Unlocked", 1);
                        PlayerPrefs.Save();
                        ...
                        PlayerPrefs.SetInt("doorOpened4", 0);
                        PlayerPrefs.Save();
