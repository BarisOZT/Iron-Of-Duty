...
public class DialogTrigger3 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        dialogCompleted = PlayerPrefs.GetInt("dialogCompleted3", 0) == 1;

        // Kapýnýn durumu
        doorOpened = PlayerPrefs.GetInt("doorOpened3", 1) == 1;
        kapý.SetActive(doorOpened);

        // Eðer s2Unlocked açýldýysa tus ve resim hep aktif kalsýn
        if (PlayerPrefs.GetInt("s2Unlocked", 0) == 1)
        {
            tus.SetActive(true);
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        PlayerPrefs.SetInt("dialogCompleted3", 1);
                        PlayerPrefs.Save();

                        // Silah 1 açýlýyor
                        PlayerPrefs.SetInt("s2Unlocked", 1);
                        PlayerPrefs.Save();
                        ...
                        PlayerPrefs.SetInt("doorOpened3", 0);
                        PlayerPrefs.Save();
